...
if isequal(chnsC,chnsX) && isequal(chnsC,chnsY)
    ...
    if ~numel(chNumbers) == numel(chMap0ind)
        ...
        % find correct order, then apply to xcoord and ycoord
    ...
    cmap        = winter(numel(chNumbers));
    f1          = figure('Name','Probe','Color','w','NumberTitle','off','Position',[295 50 649 946],'Renderer','painters');

    [~,sortChns] = sort(chNumbers);
    scatter(xcoords(sortChns),ycoords(sortChns),50,cmap,'s', 'filled');
    axis('equal')
    ...
    name        = 'KS_chanMap';
    jsonName    = fullfile(Path, [shanksName,name, name0, '.json']);
    ...
    save(fullfile(Path, [shanksName, name, name0, '.mat']), ...
        'chMap', 'connected', 'xcoords', 'ycoords', 'kcoords', 'chNumbers', 'name', 'fs')

    % SAVE figure

    saveas(f1,fullfile(Path, [shanksName, name, name0, '_probeView.fig']))
